...
function loadBracket(size) {
  ...
  xhr.onreadystatechange = function() {
    if (this.readyState == 4 && this.status == 200) {
     document.getElementById("bracketimage").setAttribute("src", "GenerateBracket.php");
     alert(this.responseText);
    }
  };
  xhr.open("POST", "GenerateBracket.php", true);
  xhr.setRequestHeader("Content-type", "application/json");
  shuffle(whitelist);
  xhr.send(JSON.stringify(whitelist.slice(0,64)));*/
